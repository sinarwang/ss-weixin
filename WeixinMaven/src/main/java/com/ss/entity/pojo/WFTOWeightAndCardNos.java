...
public class WFTOWeightAndCardNos {
	private String ProduceM_Name;
	private BigDecimal totalWeight;
	private Integer totalCardNo;
	
	/**
	 * 根据ResultSet对（WFTOWeightAndQuantity）进行初始化
	 * @param rs 结果集ResultSet
	 */
	public void initFromRs(ResultSet rs) {
		Field[] fields = WFTOWeightAndCardNos.class.getDeclaredFields();
		try {
			for(Field f : fields) {
				String fieldName = f.getName().substring(0, 1).toUpperCase() + f.getName().substring(1);
				String methodName = "set" + fieldName;
				Class fieldType = f.getType();
				Method m = WFTOWeightAndCardNos.class.getDeclaredMethod(methodName, fieldType);
	...
	public Integer getTotalCardNo() {
		return totalCardNo;
	}
	public void setTotalCardNo(Integer totalCardNo) {
		this.totalCardNo = totalCardNo;
	}
}
