	...
	wx.error(function (res) {
		...
		$(document).ready(function(){ 
			if(<%=isExist%>) {
				//alert(1);
				$.ajax({
				      url: "actionServlet?action=scrapReport",
				      datatype: "json",
				      type: "post",
				      data: {},
				      success: function (data, status, xhr) {
			    		  ...
			    		  $("#loading").remove();
			    		  var dataStaStr = "<tr><td colspan=\"2\" style=\"background:#64acdd;font-size:16px;color:#fff\">上月1日至今的生产报废数据统计</td></tr>" +
						  					"<tr>" + 
									            "<td style=\"width:50%\">总重量</td>" + 
									            "<td style=\"width:50%\">" + totalWeight + " kg</td>" + 
									        "</tr>" + 
									        "<tr>" + 
		...
		function addInfo(index, item) {
			...
			if(item.d_Quantity1 == 0) {
				str = str + "<td></td>";
			} else {
				str = str + "<td>" + item.d_Quantity1 + "</td>";
			}
			if(item.d_Weight == 0) {
				str = str + "<td></td>";
			} else {
			...
			if(item.d_Numbers == 0) {
				...
				str = str + "<td>" + item.d_Numbers + "</td>";
			}
			str = str + "<td>" + item.d_User + "</td>" + 
			            "<td>" + item.d_Date + "</td>" + 
			            "</tr>";
			$("#info").append(str);
		}
...
<div class="panel panel-success">
    <div class="panel-heading" style="background-color:#034690;">
        <h3 class="panel-title" style="color:#fff;">生产报废报表</h3>
    </div>
        ...
        <div class="table-responsive" >
            ...
            <thead>
                <tr>
                    ...
                    <th style="text-align:center">报废支数</th>
                    <th style="text-align:center">报废重量</th>
                    <th style="text-align:center">报废米数</th>
...
<div style="float:left;width:99%;margin-left:0.5%;margin-bottom:20px;"><a href="scrapReportChart.jsp?openId=<%=openId %>" class="btn btn-primary btn-block">本月生产报废数据统计</a></div>
</body>
</html>